...
{
  int i;
  _em7180 = i2c_open(_em7180, clk, sda, 0);
...
void Em7180_loadfw()
{
  ...
  if (i & 0x01)
    putStr("Barometer installed\n");
  if (i & 0x02)
    putStr("Humidity installed\n");
  if (i & 0x04)
    putStr("Temperature installed\n");
  if (i & 0x08)
    putStr("Custom Sensor\n");
  if (i & 0x10)
    putStr("Second Custom Sensor\n");
  if (i & 0x20)
    putStr("Third Custom Sensor\n");
  pause(1000);

  i = _readByte(EM7180_ADDRESS, EM7180_SentralStatus);
  if ((i & 0x01) != 0)
    putStr("EEPROM detected on the sensor bus\n");
  i = _readByte(EM7180_ADDRESS, EM7180_SentralStatus);
  if ((i & 0x02) != 0)
    putStr("EEPROM uploaded config file\n");
  i = _readByte(EM7180_ADDRESS, EM7180_SentralStatus);
  if ((i & 0x04) != 0)
    putStr("EEPROM CRC incorrect\n");
  i = _readByte(EM7180_ADDRESS, EM7180_SentralStatus);
  if ((i & 0x08) != 0)
    putStr("EM7180 in initialized state\n");
  i = _readByte(EM7180_ADDRESS, EM7180_SentralStatus);
  if ((i & 0x10) != 0)
    putStr("No EEPROM detected\n");
  int count = 0;
  while ((i & 0x01) == 0)
  {
  ...
  i = _readByte(EM7180_ADDRESS, EM7180_SentralStatus);
  if((i & 0x04) == 0x00)
    putStr("EEPROM upload successful\n");
}
...
void Em7180_setup(char accBW, char gyroBW, short accFS, short gyroFS, short magFS, char QRtDiv, char magRT, char accRT, char gyroRT, char baroRT)
{
 ...
 // EM7180 parameter adjustments
  putStr("Beginning Parameter Adjustments\n");
  ...
  putStr("Magnetometer Default Full Scale Range: +/-");
  putDec(EM7180_mag_fs);
  putStr("uT\n");
  putStr("Accelerometer Default Full Scale Range: +/-");
  putDec(EM7180_acc_fs);
  putStr("g\n");
  ...
  putStr("Gyroscope Default Full Scale Range: +/-");
  putDec(EM7180_gyro_fs);
  putStr("dps\n");
  ...
  EM7180_mag_fs = ((short)(param[1]<<8) | param[0]);
  EM7180_acc_fs = ((short)(param[3]<<8) | param[2]);
  putStr("Magnetometer New Full Scale Range: +/-");
  putDec(EM7180_mag_fs);
  putStr("uT\n");
  putStr("Accelerometer New Full Scale Range: +/-");
  putDec(EM7180_acc_fs);
  putStr("g\n");
  _writeByte(EM7180_ADDRESS, EM7180_ParamRequest, 0x4B); // Request to read  parameter 75
  ...
  EM7180_gyro_fs = ((short)(param[1]<<8) | param[0]);
  putStr("Gyroscope New Full Scale Range: +/-");
  putDec(EM7180_gyro_fs);
  putStr("dps\n");
  _writeByte(EM7180_ADDRESS, EM7180_ParamRequest, 0x00); //End parameter transfer
  _writeByte(EM7180_ADDRESS, EM7180_AlgorithmControl, 0x00); // re-enable algorithm
  ...
  if ((i & 0x01) == 0x01)
    putStr(" EM7180 run status = normal mode\n");
  i = _readByte(EM7180_ADDRESS, EM7180_AlgorithmStatus);
  if ((i & 0x01) == 0x01)
    putStr(" EM7180 standby status\n");
  if ((i & 0x02) == 0x02)
    putStr(" EM7180 algorithm slow\n");
  if ((i & 0x04) == 0x04)
    putStr(" EM7180 in stillness mode\n");
  if ((i & 0x08) == 0x08)
    putStr(" EM7180 mag calibration completed\n");
  if ((i & 0x10) == 0x10)
    putStr(" EM7180 magnetic anomaly detected\n");
  if ((i & 0x20) == 0x20)
    putStr(" EM7180 unreliable sensor data\n");
  
  i = _readByte(EM7180_ADDRESS, EM7180_PassThruStatus);
  if ((i & 0x01) == 0x01)
    putStr(" EM7180 in passthru mode!\n");
  
  i = _readByte(EM7180_ADDRESS, EM7180_EventStatus);
  if ((i & 0x01) == 0x01)
    putStr(" EM7180 CPU reset\n");
  if ((i & 0x02) == 0x02)
    putStr(" EM7180 Error\n");
  if ((i & 0x04) == 0x04)
    putStr(" EM7180 new quaternion result\n");
  if ((i & 0x08) == 0x08)
    putStr(" EM7180 new mag result\n");
  if ((i & 0x10) == 0x10)
    putStr(" EM7180 new accel result\n");
  if ((i & 0x20) == 0x20)
    putStr(" EM7180 new gyro result\n"); 
  
  pause(1000); // give some time to read the screen
  ...
  putChar('\r');
  if ((i & 0x01) == 0x01)
    putStr("Magnetometer not acknowledging!\n");
  if ((i & 0x02) == 0x02)
    putStr("Accelerometer not acknowledging!\n");
  if ((i & 0x04) == 0x04)
    putStr("Gyro not acknowledging!\n");
  if ((i & 0x10) == 0x10)
    putStr("Magnetometer ID not recognized!\n");
  if ((i & 0x20) == 0x20)
    putStr("Accelerometer ID not recognized!\n");
  if ((i & 0x40) == 0x40)
    putStr("Gyro ID not recognized!\n");
  
  putStr("Actual MagRate = ");
  i = _readByte(EM7180_ADDRESS, EM7180_ActualMagRate);
  putDec(i);
  putStr(" Hz\n");
  
  putStr("Actual AccelRate = ");
  i = 10 * _readByte(EM7180_ADDRESS, EM7180_ActualAccelRate);
  putDec(i);
  putStr(" Hz\n");
  
  putStr("Actual GyroRate = ");
  i = 10 * _readByte(EM7180_ADDRESS, EM7180_ActualGyroRate);
  putDec(i);
  putStr(" Hz\n");
  
  putStr("Actual BaroRate = ");
...
void _writeByte(unsigned char address, unsigned char subAddress, unsigned char data)
{
  ...
  printi("W Reg: %d --> %x ", subAddress, data);
}
...
uint8_t _readByte(unsigned char address, unsigned char subAddress)
{
  ...
  printi("Reg: %d --> %x ", subAddress, data);
  return data;
}
