...
void HX8357_window(short x, short y, short w, short h)
{
  ...
  shift_out(_MOSI, _CLK, MSBFIRST, 32, xa);
  writeCmd(HX8357_PASET); // set row address
  shift_out(_MOSI, _CLK, MSBFIRST, 32, xy);
...
{
  low(_CS);
  shift_out(_MOSI, _CLK, MSBFIRST, 16, color);
  high(_CS);
}
...
void HX8357_writeSChar(short x, short y, char c)
{
  ...
  for (int l=0;l<2;l++)
  {
    ...
    for (int i=0;i<4;i++)
    {
      ...
      {
        if ((v & 0x01) == 1)
          HX8357_plot(x+j, y, _fgc);
        else
          HX8357_plot(x+j, y, _bgc);
        v = v >> 1;
      }
      y++;
    }
...
void HX8357_writeChar(short x, short y, char c)
{
  ...
  for (int i=0;i<32;i++)
  {
    ...
    {
      if ((v & 0x01) == 1)
        HX8357_plot(x+j, y+i, _fgc);
      else
        HX8357_plot(x+j, y+i, _bgc);
      v = v >> 2;
    }
  }
  high(_CS);
  ...
  for (int i=0;i<64;i++)
  {
    ...
    for (int j=0;j<32;j+=2)
    {
      if ((v & 0x01) == 1)
        ...
        HX8357_plot(x+j, y+i, _bgc);
      if ((v & 0x01) == 1)
        HX8357_plot(x+j+1, y+i, _fgc);
      else
        HX8357_plot(x+j+1, y+i, _bgc);
      v = v >> 2;
    }
  }
  high(_CS);
...
{
  low(_DC);
  shift_out(_MOSI, _CLK, MSBFIRST, 8, cmd);
  high(_DC);
}
